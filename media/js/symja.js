...
function submitQuery(textarea, onfinish) {
    ...
    if (window.evaluateWasm) {
        // Call the CheerpJ bridge function instead of Ajax
        window.evaluateWasm(textarea.value).then(function(responseText) {
            textarea.ul.select('li[class!=request][class!=submitbutton]').invoke('deleteElement');
            
            // The responseText is already the JSON string from JSONBuilder
