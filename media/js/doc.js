...
function loadDoc(page) {
	if (typeof window.loadDocWasm === 'function') {
		// CheerpJ / WASM path: bypass HTTP and call the bundled Java bridge.
		window.loadDocWasm(page).then(function(jsonStr) {
			docLoaded = true;
			try {
				var response = (typeof jsonStr === 'string') ? JSON.parse(jsonStr) : jsonStr;
				showPage(response);
				var first = $$('#doc *')[0];
				if (first) first.scrollIntoView();
			} catch (e) {
				console.log('loadDoc parse error', e, jsonStr);
			}
		}).catch(function(e) { console.log('loadDocWasm failed', e); });
		return;
	}
...
function searchChange(event) {
	...
	if (!$('search').hasClassName('empty')) {
		if (query) {
			if (typeof window.searchDocWasm === 'function') {
				window.searchDocWasm(query).then(function(jsonStr) {
					docLoaded = true;
					try {
						var response = (typeof jsonStr === 'string') ? JSON.parse(jsonStr) : jsonStr;
						showPage(response);
			...
			} else if (typeof window.loadDocWasm === 'function') {
				// Fallback for older bridges without a search entry point.
				window.loadDocWasm('/' + query).then(function(jsonStr) {
					docLoaded = true;
					try {
						var response = (typeof jsonStr === 'string') ? JSON.parse(jsonStr) : jsonStr;
						showPage(response);
